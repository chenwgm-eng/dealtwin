...
with app.app_context():
    ...
    print("\n=== Test 3: 李明是决策者 ===")
    feedback3 = "原来李明是最终决策者，预算他说了算"
    result = parser.parse_feedback(1, feedback3)
    print(f"Changes: {result['total_changes']}")
    for u in result['parsed_updates']:
        print(f"  {u['stakeholder_name']} {u['attribute']}: {u['old_value']} -> {u['new_value']}")
    
    print("\n=== All State Logs ===")
    logs = parser.get_state_logs(1, limit=20)
    print(f"Total logs: {logs['total']}")
    for log in logs['logs']:
        print(f"  [{log['created_at']}] {log['change_object']} {log['attribute_name']}: {log['old_value']} -> {log['new_value']}")
